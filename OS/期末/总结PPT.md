...
| **周转时间**   | 进程从提交（*就绪*）到完成的总时间（=运行+等待） | **最小化** | 从进医院到离开医院的总时长  |
...
7. **为什么换了地址空间不需要重新定位变量？**
	...
	*   **总结：**
	    虽然换了 CR3（切换了整个页表），但因为 **“变的是用户态，不变的是内核态”**，正在运行的内核代码感觉不到地址空间发生了剧变。
...
-   **类比 / 直觉**：假设你和你朋友共用一个银行账户，余额1000元。你俩*同时*在两台ATM机上取款100元。
    1.  你的ATM机读取余额：1000元。
...
**结论**：在没有内存屏障的情况下，多线程看到的内存状态是不一致的。
...
2.  **链式分配 (Linked Allocation / Link Table)** `[S61]` `[S63]`
    *   **原理**：每个磁盘块中除了存放数据，还会留出一小块空间存放一个**指针**，指向下一个属于该文件的磁盘块。文件在目录中只需记录*起始块号*。非连续分配。
    *   **内联小图** `[Fig·S61-1]`
        ```
...
[^15]: 这里“死锁”是因为四个方向的车如果同时占用了自己右手边的资源（例如：左车占B，下车占G，右车占Y，上车占R），就会互相等待，导致死锁。图片给出的解决方案是“限制最多3辆车同时进入”，这确实能解决死锁。
...
[^23]: 
	...
	**直观理解**：
	它不关心内存是不是满了，它只关心“这个页面是不是过期了”。过期的页面会自动掉落，腾出空间。
...
[^41]: 简单来说，Peterson 算法是**用来解决两个线程互斥（Mutex）访问临界区**的纯软件算法。
	
	**一句话核心逻辑**：
	
	> Peterson 算法就是两个非常有礼貌的邻居，想共用一个洗手间时的“**举手 + 让路**”协议。
	
	为了让你彻底看懂，我们把它拆解成三个部分：**生活类比**、**代码逻辑**、以及**为什么现代 CPU 上会挂掉**。
	
	---
	
	### 1. 生活类比：举手与谦让
	
	想象线程 A 和 线程 B 是邻居，共用一个更衣室（临界区）。
	
	我们需要两个机制来保证不会两个人同时冲进去：
	
	1. **意愿（Flag）**：我想进吗？（相当于举手）
	    
	2. **谦让（Turn）**：如果咱们都想进，这次算谁的轮次？（相当于客气地说“您先请”）
	...
	**场景推演**：
	...
	- **场景 2（A 和 B 同时想进）**：
	    ...
	    - A 举手（`flag[A] = true`），B 举手（`flag[B] = true`）。
	        
	    - **关键点来了**：A 会很有礼貌地把轮次牌子翻给 B（`turn = B`），B 也会把牌子翻给 A（`turn = A`）。
	        
	    - 最后那个“翻牌子”的人是“输家”（因为他后翻，覆盖了前一个值）。
	        
	    - 假设 A 后翻了牌子（`turn` 变成了 `B`），那 A 就对自己说：“我检查 `turn`，发现 `turn == 1`（确实是 B）。我又看到 B 也举手了。得，我的谦让生效了，**A 进入等待**。”
	        
	    - B 看到牌子上写着 `turn = A`（是 B 自己写的，或者是 A 让出来的），心想：”我检查 `turn`，发现 `turn == 1`（是 B，**不是 A**）。虽然我也谦让了（曾设为 A），但显然 A 更加客气（把这事又推回来了）。既然 `turn != A`，那我就**不客气了，直接进**。“
	...
	Peterson 算法巧妙利用了 `flag` 数组和 `turn` 变量。
	
	```C++
	// 两个线程：ID 分别为 0 和 1
	bool flag[2] = {false, false}; // 表示“我想进”
	int turn = 0;                  // 表示“轮到谁优先”
	
	void entry_protocol(int id) {
	    int other = 1 - id; // 对方的 ID
	
	    // 1. 举手：宣布我想进入
	    flag[id] = true;
	
	    // 2. 谦让：假设现在轮到对方（非常有礼貌）
	    // 这行代码最关键：通过写入 turn，把自己置于“等待地位”
	    turn = other; 
	
	    // 3. 检查：
	    // 条件 A: 对方也想进 (flag[other] == true)
	    // 条件 B: 确实轮到对方了 (turn == other)
	    // 只有当 A 和 B 都满足时，我才死循环等待
	...
	它不需要任何特殊的硬件指令（比如 CAS、Test-and-Set），仅仅用了普通的读写操作，就实现了由两个线程间的互斥，而且还解决了“死锁”和“饥饿”问题。
	
	---
	
	### 3. 结合你之前的 Context：为什么它现在“失效”了？
	...
	> ...
	> 
	> 失效原因：现代 CPU 经常发生 Store-Load 重排序。
	...
	在 Peterson 算法的逻辑里，**顺序至关重要**：
	
	- 我必须**先**告诉全世界“我要进”（写 flag）。
	    
	- **然后**再去检查“有没有人已经在里面了”（读 other flag）。
	    
	
	现代 CPU 的乱序执行（Out-of-Order Execution）是这样的：
	
	CPU 觉得：“写 flag 是写内存，太慢了，我先放进写缓冲（Store Buffer）里稍后慢慢写。但是读 flag[1] 不需要等待，我现在就去读。”
	
	**于是悲剧发生了**：
	
	1. **线程 0**：把 `flag[0]=true` 扔进写缓冲（内存里还没变），紧接着去读 `flag[1]`。内存里 `flag[1]` 是 false。线程 0 觉得没人，**进屋了**。
	    
	2. **线程 1**：把 `flag[1]=true` 扔进写缓冲（内存里还没变），紧接着去读 `flag[0]`。内存里 `flag[0]` 也是 false（因为线程 0 的写入还在缓冲里没刷出来）。线程 1 觉得没人，**也进屋了**。
	    
	
	**结果**：两个线程同时进入临界区，互斥失效，数据写烂。
	
	### 总结
	
	1. **Peterson 算法是啥**：一个不需要硬件锁支持的、纯逻辑的、双线程互斥算法。
	    
	2. **核心思想**：我想进（Flag）+ 你先请（Turn）。
	    
	3. **现代编程怎么做**：必须加**内存屏障（Memory Barrier）**。也就是在“写 Flag”和“读 Other Flag”之间，强制 CPU 把写缓冲的数据刷入内存，在这个动作完成前，不许执行后面的读操作。
