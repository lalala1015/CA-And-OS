...
我们已经掌握了 Cache 的基本工作原理和放置策略，接下来，我们继续探讨影响 Cache 性能的其他关键参数，以及替换和写入策略。
...
*   **🧠 常见策略 (cache界的“断舍离”法则)** `[S28]`：
    ...
    4.  **🤏 NMRU (非最近最少使用 - Not Most Recently Used)** `[S28]`
        - **机制**：LRU 的**近似算法**。只保证**不替换**最近刚刚访问过的那个块 (MRU)，在剩下的块中（通常是随机）选择一个替换。
        - **硬件代价**：中等偏低。通常只需 1 个 bit 来标记 MRU 位置。
        - **特例**：在 **2路组相联 (2-way Associative)** 中，不是 A 就是 B，只要保护了 MRU，剩下的那个必然是 LRU。因此在 2-way Cache 中，**NMRU 等价于 LRU**。

    5.  **🔮 Belady's 最优策略 (上帝视角)** `[S28]`
        *   **怎么做**：踢掉那个**在未来最长时间内**都不会被用到的块。
        *   **评价**：这是**作弊**！现实中没人能预知未来（除了上帝）。
        *   **用途**：作为**基准 (Baseline)**。通过 Trace 模拟运行得出最优结果，用来评估 LRU 等实际算法与理论极限的差距（即 Miss Rate 还能优化多少）。
...
这是最直接的“加资源”手段。
- **Compulsory misses (强制性未命中) [`=`]**
    
    - **原因**：**无关**。强制性未命中是因为数据**第一次**进入 Cache。无论你的 Cache 有多大，第一次访问某个从来没见过的数据块时，它肯定不在里面。
        
- **Capacity misses (容量未命中) [`↓`]**
    - **原因**：**显著降低**。容量未命中的定义就是“Cache 太小，装不下整个工作集 (Working Set)”。Cache 变大，能容纳的数据更多，被挤出的概率自然降低。
        
- **Conflict misses (冲突未命中) [`↓`]**
    - **原因**：**降低**。在关联度不变的情况下，Cache 变大意味着有更多的组 (Sets)。地址映射会更加分散，两个不同地址映射到同一个组并发生“碰撞”的概率变小。
- **Hit latency (命中延迟) [`↑`]**
    - **原因**：**升高**。这是物理电路决定的。Cache 越大，其内部的存储阵列、解码器电路就越大，信号传输的物理距离变长，门电路级数增加，导致查找和读取数据的速度变慢。
- **Miss latency (缺失延迟) [`=`]**
    - **原因**：**不变**。Miss Latency 取决于**下一级存储器**（如主存）的速度。无论 Cache 有多大，一旦 Miss，都要去主存搬数据，这个搬运时间由主存决定，与 Cache 大小无关。
...
- **Compulsory misses (强制性未命中) [`=`]**
    - **原因**：**无关**。同上，关联度不改变数据初次加载的需求。
- **Capacity misses (容量未命中) [`=`]**
    - **原因**：**无关**。严格定义上，容量未命中是指“即使在全相联 Cache 中也会发生的未命中”。它只跟容量有关，跟怎么放（关联度）无关。
- **Conflict misses (冲突未命中) [`↓`]**
    - **原因**：**显著降低**。这是提高关联度的核心目的。更多的路数（Ways）意味着同一个组内可以容纳更多“抢地盘”的数据块，减少了因哈希冲突导致的颠簸。
- **Hit latency (命中延迟) [`↑`]**
    - **原因**：**升高**。关联度越高，硬件需要并行的**比较器 (Comparator)** 就越多，且通过多路选择器 (MUX) 选择数据的逻辑越复杂，增加了电路延迟。
- **Miss latency (缺失延迟) [`=`]**
    - **原因**：**不变**。同上，去主存取数据的时间主要由主存决定。
---

 **3. 增加块大小 (Increase Block Size)**

即每次从内存搬运更多的数据（例如从 32B 增加到 64B）。

- **Compulsory misses (强制性未命中) [`↓`]**
    
    - **原因**：**降低**。利用了**空间局部性 (Spatial Locality)**。因为块大了，当你取一个数据时，顺便把它周边的邻居也都带进来了。下次访问邻居时（大概率事件），它们已经在 Cache 里了，从而消除了原本会发生的强制性未命中。
        
- **Capacity misses (容量未命中) [`↓`]**
    
    - **原因**：**降低**。这主要得益于空间局部性的提升。虽然块变大可能导致 Cache 行数减少，但对于具有良好空间局部性的程序，大块能更有效地覆盖“活跃数据区”，使得在有限容量下的有效命中率提升。
        
- **Conflict misses (冲突未命中) [`?`]**
    
    - **原因**：**不确定 (Trade-off)**。这是一个博弈过程：
        - **利**：空间局部性好，减少了部分冲突。
        - **弊**：总容量不变时，**块越大，块的数量（行数）就越少**。这意味着更多的地址会映射到同一个组，增加了冲突概率。
        - **弊**：**Cache 污染 (Pollution)**。如果块太大，可能搬进来很多根本用不到的“垃圾数据”，占据了宝贵空间，把有用数据挤走。
        - _结论_：通常随着 Block Size 增大，Conflict Misses 会先降后升，呈现“U型”曲线，所以打问号。
            
- **Hit latency (命中延迟) [`=`]**
    
    - **原因**：**基本不变**。命中时间主要花在 Tag 比较上。无论块后面带的数据有多长，Tag 的比对速度基本一致。
        
- **Miss latency (缺失延迟) [`↑`]**
    
    - **原因**：**升高**。**传输惩罚 (Transfer Penalty)**。从主存搬运 64B 数据肯定比搬运 16B 数据花的时间长（受限于总线带宽）。

*   **一句话带走**：
    提升 Cache 性能没有银弹，每种药都有副作用。
...
- 分类与处理流程：
    ...
    - **情况 B：真缺页 (True Page Fault / Major Fault)**
        ...
        - **OS 处理流程 (The Heavy Lifting)**：
            1. **陷入内核**：保存当前进程上下文。
            2. **分配物理页框 (Physical Frame)**：
                - 如果物理内存已满，运行 **页面置换算法 (Page Replacement Algorithm)**（如 LRU, Clock），选择一个受害者页面 (Victim Page) 驱逐。
                - 如果受害者页面是**脏的 (Dirty Bit = 1)**，需先将其**写回磁盘** (Write Back)。
            3. **磁盘 I/O**：启动 DMA，将目标页面从磁盘**读取**到刚才腾出的物理页框中。（**极慢**，毫秒级延迟）。
            4. **进程调度**：在等待磁盘 I/O 期间，CPU 会**切换 (Context Switch)** 去运行其他进程（Blocked -> Ready）。
            5. **更新映射**：I/O 完成后，修改页表项 (Valid Bit 置 1)，更新 PPN。
            6. **重试指令**：恢复原进程上下文，重新执行那条产生缺页的指令。
---
...
*   **问题如何产生？** `[S55]`
    ...
    *   `VA1 -> PA`
    *   `VA2 -> PA`
	-   意味着，VA1和VA2的偏移值Page Offset是一样的，如果index位数在offset的范围内的话，应该会映射到同一个Index。现在我们考虑：
    如果 VA1 和 VA2 的 `Index` 位不同（当 Cache Index 超出 Page Offset 范围时可能发生，*虽然前文提到不允许如此*，但是我们这里考虑*被迫*Index超出Offset的情况），那么：
...
*   **解决方案** `[S56]` `[S60]`：
    1.  **软件层面（OS 约束）** `[S56]`：
        *   **页面着色 (Page Coloring)**：操作系统在进行虚拟到物理映射时，强制要求所有共享同一个物理页的虚拟地址，它们的 `Index` 位必须相同。这样它们就总是会映射到 Cache 的同一个 Set，从根本上避免了多个副本的产生。这是啥意思？如何做到Index都保持相同的？[^9]
...
[^9]: 当 `Cache Way 大小 > 页面大小` 时，硬件推导虽然不满足“无别名”条件，但我们可以通过**软件（操作系统）**的手段来弥补硬件的“缺陷”。
