...
*   **🕒 利用时间局部性** `[S15]`
    *   凡是刚被访问过的数据，不要急着扔回慢速的大内存里，而是先把它**抄一份留在手边的 Cache（书桌）上**。`[S15]`
...
#### **知识卡片：如何将主存的物理地址映射到 Cache 的存储阵列中**

---

##### **1. Cache 寻址原理：如何精准定位？** `[S19]`

当 CPU 发出一个内存读写请求时，发送的是一个 32位或 64位的物理地址。Cache 控制器会将这个地址在逻辑上切分为三段：

$$\text{Address} = \mathbf{Tag} \parallel \mathbf{Index} \parallel \mathbf{Offset}$$


- **Tag (标记位)**：地址的高位。
    
    - **作用**：用于**身份验证**。因为 Cache 容量远小于主存，不同的主存块可能会映射到同一个 Cache 位置，Tag 用于区分当前 Cache 里存的到底是哪一块内存的数据。
        
- **Index (索引位)**：地址的中间位。
    
    - **作用**：用于**寻址定位**。它直接对应 Cache 的“行号”或“组号”。硬件上，这几位通常直接连接到 SRAM 的地址解码器（Decoder）。
        
- **Offset (块内偏移)**：地址的低位。
    
    - **作用**：用于**数据截取**。Cache 存取是以“块 (Block/Line)”为单位的（例如 64 Bytes），Offset 用于从这 64 Bytes 中选出 CPU 具体需要的那个字 (Word) 或字节 (Byte)。
...
这三种策略的本质区别在于：**主存块地址 $j$ 映射到 Cache 行号 $i$ 的数学规则不同**，以及由此带来的硬件比较器数量差异。[注意这个 j 代表的是块号，而不是字节地址]

假设：Cache 总行数为 $C$，每组包含的行数为 $N$（也就是 N路），组数 $S = C/N$。
...
*   *映射规则*：
    ...
    - 主存中的某个块，只能映射到 Cache 中**唯一**的一个固定行。
- **硬件行为**：
    1. 根据 **Index** 选中 Cache 中的唯一一行。
    2. 读取该行的 Valid Bit 和 Tag。
    3. **比较器数量：1 个**。将读取的 Tag 与地址中的 Tag 进行比较。
*   **地址划分**：`Tag | Index | Offset`
*   **优点**：
    *   **快**！不用犹豫，直接去那个格子看一眼，有就有，没有就没有，无需多路选择。
*   **缺点**：**冲突未命中 (Conflict Misses)**
    - **抖动 (Thrashing)**：如果程序交替访问两个映射到同一 Index 的不同内存块，会导致 Cache 频繁换入换出，命中率急剧下降。
...
- **映射规则**(S是组数，得到的SetIndex是组号)：
    
    $$Set Index=j\mod S$$
    
    - 主存中的某个块，可以映射到 Cache 中**特定组 (Set)** 内的**任意**一行（Way）。
    - 常见的有 2-way, 4-way, 8-way 等。
- **硬件行为**：
    1. 根据 **Index** 选中 Cache 中的某一个**组 (Set)**。
    2. 同时激活该组内的 $N$ 个行（Way）。
    3. **比较器数量：$N$ 个**。硬件并行读取这 $N$ 行的 Tag，并同时与地址 Tag 进行比较。
    4. 通过一个 $N$ 选 1 的多路选择器 (MUX) 输出命中的数据。
*   **地址划分**：`Tag | Set Index | Offset`
*   **优点**：`[S22]`
    *   **不怎么打架了**！两个Cache块虽然都去第 5 组，但可以一个坐左边，一个坐右边，和平共处。
*   **缺点**：`[S22]` `[S23]`
    *   **变慢了一点点**。因为到了第 5 组，你得同时检查组里的 N 个位置，看哪一个才是你要的书（硬件需要 N 个比较器并行工作）。
    *   **折中方案**。通过增加硬件复杂度（$N$ 个比较器），显著降低了冲突未命中 (Conflict Misses) 的概率。现代 CPU L1/L2 Cache 多采用此方式（如 8-路组相联）。
* 举例[^1]

###### **策略 3：全相联 (Full Associativity) —— “随便坐”** `[S24]`
*   内存块可以放在Cache的**任意位置**（通常用于TLB）
*   **地址划分**：`Tag | Offset` (不需要 Index 了，因为不需要定位去哪一组)
-   **硬件行为**：
    - **比较器数量：$C$ 个（所有行）**。
    - 必须使用 **CAM (内容寻址存储器)** 电路，或者在逻辑上并行比较 Cache 中**所有**有效行的 Tag。
*   **优点**：
    *   **完全没冲突**。只要书架没被彻底塞满，新书来了永远有地方放，不会因为“抢位置”被踢出去。
    *  **命中率最高**（没有冲突未命中，只有容量未命中）。
*   **缺点**：`[S24]`
    *   **找书太累了**！因为书可能在任何角落，每次找书都要把书架上**所有**的书挨个（或同时）检查一遍 Tag。
    *  **电路复杂，功耗极高，速度慢**。通常仅用于容量极小但对命中率要求极高的部件，如 **TLB (快表)**。
...
*   **要解决的问题（直觉）**：
    Cache的不同参数（容量、关联度、块大小）会极大地影响最终的性能表现。我们需要理解调整这些参数会带来哪些正面和负面的影响。

*   **参数 1：容量 (Capacity)** `[S25]`
    *   **影响**：Cache 的总大小。
    *   **正面影响**：
        *    更大的容量可以容纳程序更多的“工作集 (working set)”——即程序在一段时间内频繁访问的数据集合。
        *   能更好地利用时间局部性，从而降低 **Cache Miss 率**。
    *   **负面影响**：
        *    **增加访问延迟**：越大的存储器，其内部走线越长、解码逻辑越复杂，导致访问速度变慢（“越小越快，越大越慢”）。
        *   访问延迟的增加可能会延长处理器的关键路径，从而限制时钟频率的提升。
        *   成本和功耗更高。
    *   **内联小图（Miss 率 vs 容量）**：
![[Pasted image 20251231175410.png]]
        *   **[Fig·S25-1]** 未命中率与 Cache 容量的关系图。随着容量增加，未命中率迅速下降，但当容量超过程序的工作集大小后，继续增加容量带来的收益会递减。

*   **参数 2：关联度 (Associativity)** `[S26]`
    *   **影响**：每个组 (set) 里能放几个块。
    *   **直觉**：直接映射是“每本书都有唯一指定位置”，N路组相联是“某一类书可以放在指定的N个位置中的任意一个”。
    *   **正面影响**：
        *    ++ **命中率越高**：更高的关联度提供了更多的放置选择，能有效降低**冲突未命中**。
    *   **负面影响**：
        *    -- **访问时间更长**：需要并行比较组内所有块的 Tag，比较器和后续的多路选择器 (MUX) 逻辑更复杂，延迟更高。
        *    -- **硬件更复杂**：需要更多的 Tag 比较器。
...
| **类型**  | **英文名**        | **根本原因**                                                   | **别名**   | **怎么解决？**               |
| ------- | -------------- | ---------------------------------------------------------- | -------- | ----------------------- |
| **强制性** | **C**ompulsory | **第一次访问**。块从来没进过 Cache。                                    | 冷启动 Miss | **增大块大小** (预取)          |
| **容量性** | **C**apacity   | **Cache 太小了**。程序需要的数据总量超过了 Cache 总大小，导致旧数据不得不被踢出去，下次用又得重抓。 | -        | **增大 Cache 容量**         |
| **冲突性** | **C**onflict   | **映射位置不够**。虽然 Cache 还有空位，但大家非要抢同一个 Set 或同一个行，导致被挤出去。       | 碰撞 Miss  | **增加相联度** (如从直接映射改为组相联) |
        *   **负面影响：产生干扰 (Interference)** `[S27]`
            *    **降低了块的数量**：在总容量不变的情况下，块越大，Cache 能容纳的总块数就越少。这可能会增加冲突，因为更多不同的内存地址会竞争更少的 Cache 块。
            *    **带宽浪费/污染**：如果程序空间局部性差，取回一个大块，但只用了其中的一小部分数据，那么大部分传输带宽和宝贵的 Cache 空间都被浪费了，这称为 **Cache 污染**。
*   **内联小图（Miss 率 vs 块大小）**：
...
- **触发条件**：发生 Cache Miss，且目标**组 (Set) 已满**（所有 Way 都有有效数据）。
	
- **动作**：必须根据特定算法选择一个现有的块（Block/Line）作为**受害者 (Victim)** 移出 Cache，以便为从下级存储取回的新数据腾出空间。
	
- **适用范围**：仅适用于**组相联 (Set Associative)** 和 **全相联 (Fully Associative)** 映射。
	- _注_：**直接映射 (Direct Mapped)** 不需要替换策略，因为每个地址只能映射到唯一的一个位置，直接覆盖即可。

*   **🧠 常见策略 (cache界的“断舍离”法则)** `[S28]`：

    1.  **🎲 随机 (Random)**
        *   **怎么做**：闭着眼睛随便抓一个扔出去。
        * **硬件代价**：极低。不需要记录任何历史访问状态。
        *   **特性**：无法利用局部性原理。但在高组相联度（High Associativity）的大容量 Cache 中，其表现往往出人意料地接近 LRU，且无边缘情况抖动风险。

    2.  **🕰️ FIFO (先进先出 - First-In, First-Out)**
          - **机制**：维护一个循环队列或计数器，替换掉**最早进入**该组的块。
        - **硬件代价**：低。每个组只需维护一个指向“最老”块的指针。
        - **缺陷**：严重违背**时间局部性 (Temporal Locality)**。
            - 程序中常有一些初始化加载后一直频繁访问的“热数据”（如常用的全局变量、内核代码）。FIFO 会因为它们“入队时间早”而将其踢出，导致性能恶化。

    3.  **📉 LRU (最近最少使用 - Least Recently Used)** `[S28]`
          - **机制**：替换掉**最长时间未被 CPU 访问**的块。
        - **原理**：利用**时间局部性**——如果一个数据刚刚被访问过，那么它在不久的将来很可能再次被访问。
        - **硬件代价**：**极高**。
            - 必须在**每次** Cache Hit 或 Miss 时更新该组内所有块的访问状态（Age）。
            - 对于 $N$ 路组相联，完全实现 LRU 通常需要 $N \times N$ 的矩阵或复杂的堆栈逻辑。当 $N > 4$ 时，硬件开销通常变得不可接受。

---

#### **知识卡片：写策略 (Write Policy) —— 也可以叫“文件修改守则”**

*   场景：Cache 中的数据只是主存数据的副本。
*   冲突：当 CPU 执行写操作（Store 指令）修改了 Cache 中的数据后，Cache 里的新值和主存里的旧值就出现了不一致。
*   决策：写策略决定了何时将这个更新同步回主存。

这分为两种情况：
1.  **写命中**：CPU 要写入的地址**已经存在**于 Cache 中
2.  **写未命中**：即 CPU 要写入的地址**不在** Cache 中
...
*   **性能提升三大路径** `[S33]` `[S34]`：
    从上面的公式可以看出，要提升 Cache 性能，我们有三条路可走：
...
*   **要解决的问题**：
    为了有效降低 Miss 率，我们首先需要搞清楚 Miss 是怎么来的。

*   **1. 强制性未命中 (Compulsory Miss)** `[S35]`
    *   **定义**：`[S35]` 第一次访问一个块时，由于该块从未被加载进 Cache，必然发生的 Miss。也叫“冷启动未命中 (Cold-Start Miss)”。
    *   **如何减少**：
...
**1. 增大 Cache 容量 (Increase Cache Size)**

这是最直接的“加资源”手段。

- **核心正面影响（主要目的）**：**降低 Miss Rate**。
    
    - 容量大了，能装的数据自然就多了，主要显著降低了**容量未命中 (Capacity Misses)** 和冲突未命中。
        
- **核心负面影响（副作用）**：**增加 Hit Time (命中时间) 和 成本**。
    
    - Cache 越大，物理电路越大，查找路径变长，导致判断是否命中的速度变慢。
        

---

**2. 增加关联度 (Increase Associativity)**

即从“直接映射”变为“组相联”，或提高组相联的路数（Ways）。

- **核心正面影响（主要目的）**：**降低 Conflict Misses (冲突未命中)**。
    
    - 这是提高关联度的唯一原因。它减少了因哈希冲突（多个地址抢同一个位置）导致的颠簸。
        
- **核心负面影响（副作用）**：**增加 Hit Time (命中时间)**。
    
    - 硬件需要更多的比较器并行工作，且多路选择逻辑变复杂，导致电路延迟增加。
        

---

**3. 增加块大小 (Increase Block Size)**

即每次从内存搬运更多的数据（例如 32B $\to$ 64B）。

- **核心正面影响（主要目的）**：**利用空间局部性 (Spatial Locality)**。
    
    - 一次搬运更多邻近数据，大幅降低**强制性未命中 (Compulsory Misses)**。
        
- **核心负面影响（副作用）**：**增加 Miss Penalty (缺失惩罚)**。
    
    - 块越大，从主存搬运数据所需的时间（传输时间）就越长。
        
    - _注：块过大还可能导致 Cache 污染（搬入无用数据），反而导致 Miss 率上升。_
...
1. **大容量**：装得多（Miss少），但找得慢（Hit Time长）。
    
2. **高关联**：冲突少（Conflict Miss少），但电路复杂（Hit Time长）。
    
3. **大块头**：邻居多（利用空间局部性），但搬运慢（Miss Penalty长）。
...
*   **关键组件与流程** `[S40]` `[S41]`：
    1.  **虚拟地址 (VA)** `[S41]`：程序中产生的地址（比如指针的值）。 每个进程都认为自己独占一个巨大的地址空间，例如在64位系统上是 $2^{64}$ 字节。
    2.  **物理地址 (PA)** `[S41]`：物理内存芯片上真实的地址。 其大小受限于实际安装的内存条容量。
    3.  **地址转换 (Address Translation)** `[S43]`：核心机制，将虚拟地址转换为物理地址。这个过程由硬件（MMU - 内存管理单元）在每次访存前自动完成。
...
*   **1. 程序隔离 / 保护** ：
    *   每个进程都在自己的“沙箱”（私有虚拟地址空间）里运行，无法直接访问其他进程的物理内存。这大大提高了系统的稳定性和安全性。
    *   每个页还可以设置**读/写/执行权限**，由硬件强制保证。例如，代码段可以设为只读+可执行，防止被意外修改。

*   **2. 高效的内存使用**：
    *   **内存共享** ：可以将同一个物理页映射到多个进程的虚拟地址空间，实现高效的进程间通信或共享库（如 C 语言库）。
...
*   **页表大小计算** `[S46]`：
    ...
    *   **示例** `[S46]`：
    ...
    `[S46]` 对于一个 32-bit 系统，每个进程 4MB 的页表可能还算可以接受。但如果是 **64-bit 系统**呢？虚拟页的数量会变成天文数字，单级页表会大到完全不切实际！解决方案：多级页表[考试不涉及！]
...
*   **解决方案：TLB** `[S51]`
    ...
    *   **形式化表述（严格）** `[S51]`：
        **TLB (Translation Lookaside Buffer)** 是一个**专门用于缓存近期用过的虚拟页号 (VPN) 到物理页号 (PPN) 映射关系**的小型、高速硬件缓存。它本质上是**页表的一个 Cache**。[^6]详细讲解他与Cache的区别！
    ...
    *   **TLB 的特性** `[S51]`：
        *   **容量小**：通常只有 16-512 个条目。
        *   **关联度高**：通常是全相联或高路组相联，以减少冲突。(类似于Cache)[^7]
        *   利用的是**页表访问的时间局部性**。
...
*   **结论：**
    现在大家更喜欢**硬件方式**，因为**快**才是王道！

---

##### 2. 缺页异常 (Page Faults) - (小纸条没有，书架上也没有)

这种情况叫“缺页”。这是个**大麻烦**。这种情况发生在：**TLB 和内存的页表中都无法访问该目标页面。**

*   **这是什么意思？**
    查找到页表项 (PTE) 后，发现 有效位 (Present/Valid Bit) = 0。这意味着该虚拟页面当前不在物理内存 (DRAM) 中。
    *   **情况A（Segmentation Fault）：** 这书压根就不存在，你记错名字了。-> **直接报错崩溃**（这就是为什么编程经常遇到段错误）。
    *   **情况B（True Page Fault）：** 书是有的，但是书架放不下了，被搬到 **“地下仓库” (硬盘)** 去了。

- 分类与处理流程：
    这是必须由 操作系统 (OS) 处理的精确异常 (Precise Exception)。
    - **情况 A：非法访问 (Segmentation Fault)**
        - **原因**：该虚拟地址未被分配（不在 VMA 链表中），或权限错误（如尝试写只读段）。
        - **结果**：OS 向进程发送 `SIGSEGV` 信号，通常导致进程终止 (Core Dump)。
    - **情况 B：真缺页 (True Page Fault / Major Fault)**
        - **原因**：页面是合法的，但由于内存不足，之前被**换出 (Swapped out)** 到了磁盘。
        - **OS 处理流程 (The Heavy Lifting)**：
		    负责将页面从磁盘调入内存，[具体流程略了！]
---
...
*   **形式化表述（严格）** `[S62]`：
    乱序执行，也称为**指令动态调度**，是一种允许处理器在运行时，不按照程序原始的指令顺序来执行指令的微体系结构技术。其核心思想是，只要指令的操作数准备就绪，并且有可用的执行单元，该指令就可以被执行，无需等待在它前面但尚未就绪的指令。
...
*   **简单描述流程** `[S62]`：
    ...
    5.  **顺序提交 (In-Order Commit)** (注：这是完整 OoO 处理器的一部分，Tomasulo 算法本身不包含，但很重要)：为了保证程序的正确语义（精确异常），结果通常会按原始指令顺序写回寄存器或内存。
...
*   **核心思想**：
    Tomasulo 算法是一种经典的动态调度算法，它完美地实现了上述乱序执行的四个条件。

*   **两大核心机制** `[S64]`：
    1.  **分布式保留站 (Reservation Station)**：`[S64]` 每个计算单元（如加法器、乘法器）前都有一组自己的保留站。指令根据其操作类型被分发到相应的保留站队列中。
    2.  **通过寄存器重命名消除伪相关 (WAW & WAR)** `[S64]`：
        * 通过将指令的目标寄存器“重命名”为将要产生该结果的**保留站的 ID (tag)**，从而彻底消除了 WAR 和 WAW 这种因为寄存器名不够用而产生的伪相关，只保留了真正的 RAW 数据依赖。
...
* ![[Pasted image 20260101205621.png]]
    ...
    *   **寄存器结果状态 (Register Result Status)**：
        *   可以看作是寄存器文件的扩展。 每个寄存器除了有 `value` 字段，还有一个 `st` (status/tag) 字段。
...
*   **指令序列**:
    ...
    3.  `add f0, f1, f2`
    4.  `mul f3, f0, f2`
*   **寄存器**: `f1`=1.0, `f2`=1.0。寄存器的 `st` 字段初始均为空（表示寄存器的值是最新的）。
*   **保留站**: 所有保留站都为空。
...
1. **发射 (Issue/Dispatch)**：
    ...
    - **发送到保留站**：如果保留站有空，就把指令发过去。如果操作数在寄存器里，就读出来；如果操作数在 ROB 里（还没提交），就建立依赖关系。（**即来源从保留站ID换成了ROB**）
		- `I1: R1 = R2 + R3` 分配 ROB#5  
		    → `RenameMap[R1]=5`
		- `I2: R4 = R1 + 1` 分配 ROB#6  
		    读 R1 时查表发现 `RenameMap[R1]=5`  
		    → I2 在保留站里记录 `Qj=ROB#5`（等 I1）
...
*   **要解决的问题（直觉）**：
    在深深的流水线中，当我们遇到一个条件分支指令（比如 `if (x > 0) ...`），我们必须等到这个条件被计算出来（通常在流水线的执行EX阶段），才能确定下一条指令应该从哪里取。在等待的这几个周期里，取指单元（IF）无事可做，整个流水线都“饿”着，这被称为**控制冒险 (Control Hazard)**。
...
*   **定义**：
    静态预测是指*预测策略是固定的*，不随程序运行时的行为而改变。
*   **常见策略**：
    *   **永远预测不跳转 (Predict Not Taken)**：简单粗暴，假设所有分支都不会发生跳转。对于大部分循环的判断条件（如 `for` 循环的 `i < 100`）来说，这种预测在循环体内是正确的。
    *   **永远预测跳转 (Predict Taken)**：与上面相反。
    *   **向后跳转预测跳转，向前跳转预测不跳转 (Backward Taken, Forward Not Taken - BTFNT)**：这是一种基于经验的启发式策略。编译器通常把循环的判断分支放在循环体的末尾，这是一个“向后”的跳转，它绝大多数时候都会发生。而 `if-else` 结构中的分支通常是“向前”的跳转。[这里的方向，指的是**跳转的目标地址**相对于**当前指令地址**的位置]
		- **向后 (Backward) = 回头 = 回到过去 = 地址变小**
		- **向前 (Forward) = 前进 = 去往未来 = 地址变大**
...
*   **内联小图（2-bit 预测器状态机）**：
    ...
    ```
    *   **[Fig·S78-1]** 2-bit 饱和计数器状态转换图。 清晰地展示了四个状态以及它们如何根据实际的分支结果进行转换。从一个“强”状态（11或00）需要连续两次相反的结果才能翻转到另一个预测区。

*   **性能分析 (回到循环例子)** `[S79]`：
    *   `j<3` 的行为序列：**T, T, T, N**...
    *   **稳定后**: 计数器会稳定在“强跳转”(11)状态。
        *   **第1次 T**: 状态10, 预测 T, 实际 T -> 猜对。
...
- 工作方式
	- 在一个时钟周期内发射多条指令，由多个执行单元并行处理这些指令。
	- 动态调度：硬件在运行时对指令进行调度和重排，提高并行性
*   **优点**：
...
*   **这是什么？**
    超长指令字是把难题抛给了**编译器**。
    它要求在程序编译的时候，就把能并行的指令找出来，硬拼成一条非常长的“巨型指令”。

*   **特点**：所有的累活、脑力活都由**编译器** 在编译时完成。
...
*   **定义**：最严格、最符合程序员直觉的内存模型。
    一个系统被称为顺序一致的，如果其任何执行结果，都等同于“**所有处理器的操作，以某种单一的全局顺序（global total order）交错执行，并且每个处理器内部的操作顺序，都遵循其程序本身指定的顺序**”。
...
*   **解决方案：掩码与串行化 (Masking and Serialization)** `[S134]`
    ...
    2.  **路径一**：硬件会生成一个**活动掩码 (Active Mask)**，只激活那些需要走 `if` 路径的线程，其他线程则被“关闭”，不执行指令，也不改变状态。然后执行 `if` 块内的所有代码。
    3.  **路径二**：`if` 路径执行完毕后，硬件反转掩码，激活那些需要走 `else` 路径的线程，关闭其他线程，然后执行 `else` 块内的所有代码。
    4.  **重新汇合**：两条路径都执行完后，所有线程重新激活，继续执行后面的公共代码。
...
[^8]: 为了理解推导，我们需要把**虚拟地址**和**Cache 索引机制**放在一起看。
